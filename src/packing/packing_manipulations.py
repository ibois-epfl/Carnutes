...
def match_skeletons(model_element: utils.geometry.Pointcloud, original_skeleton: utils.geometry.Pointcloud) -> utils.geometry.Pointcloud:
    """"
    ...
    for i in range(len(model_element.points)-1):
        dist = np.linalg.norm(np.array(model_element.points[i]) - np.array(model_element.points[i+1]))
        list_of_reference_distances.append(dist)
    list_of_target_distances = []
    for i in range(len(original_skeleton.points)-1):
        dist = np.linalg.norm(np.array(original_skeleton.points[i]) - np.array(original_skeleton.points[i+1]))
        list_of_target_distances.append(dist)
    new_skeleton_as_list = [[original_skeleton.points[0][0],
    ...
    if np.sum(list_of_target_distances) < np.sum(list_of_reference_distances):
        ...
        for i in range(len(list_of_reference_distances)):
            ...
            new_point =  [original_skeleton.points[j][0] + ratio * (original_skeleton.points[j+1][0] - original_skeleton.points[j][0]), 
                          original_skeleton.points[j][1] + ratio * (original_skeleton.points[j+1][1] - original_skeleton.points[j][1]), 
                          original_skeleton.points[j][2] + ratio * (original_skeleton.points[j+1][2] - original_skeleton.points[j][2])]
